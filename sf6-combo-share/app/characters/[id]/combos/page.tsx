// app/characters/[id]/combos/page.tsx
...
export default async function CharacterCombosPage({
  ...
  // 安定ソート（同値時は createdAt desc で決める）
  const orderBy =
    sort === "damage"
  ...
  const sortLink = (key: SortKey) => {
    ...
    return `/characters/${characterId}/combos?sort=${key}&dir=${next}`;
  };
  ...
  return (
    <main style={{ maxWidth: "1200px", margin: "20px auto", padding: "0 16px 40px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "end", gap: "12px" }}>
        <div>
          <h1 style={{ marginBottom: "4px" }}>{character.name} のコンボ一覧</h1>
      ...
      <div style={{ marginTop: "14px", border: "1px solid #e0e0e0", borderRadius: "8px", overflow: "hidden" }}>
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "13px" }}>
          ...
          <tbody>
            {combos.length === 0 ? (
              <tr>
                <td colSpan={9} style={{ padding: "14px 10px", color: "#777" }}>
                  このキャラのコンボがまだありません。
                </td>
              </tr>
            ) : (
              combos.map((combo: any) => {
                ...
                return (
                  <tr key={combo.id} style={{ borderTop: "1px solid #eee" }}>
                    ...
                    <td style={{ padding: "10px 8px", verticalAlign: "top", maxWidth: "240px" }}>
      ...
      <div style={{ marginTop: "10px", fontSize: "12px", color: "#777" }}>
        ソート：投稿日 / ダメージ / Dゲージ / SAゲージ（列名クリック）
      </div>
    </main>
  );
}
